...
module.exports = function (app) {
  ...
  app.defineProp('Router', routes().Router);
  app.defineProp('Route', routes().Route);
  ...
  app.defineProp('lazyrouter', function() {
    if (!this.router) {
      app.defineProp('router', new Router({
  ...
  app.defineProp('handle', function(method, file, done) {
    ...
    if (typeof done !== 'function') {
      done = this.handleError(method, file);
    }
  ...
  app.defineProp('dispatch', function(method, file, fns) {
    for (var key in file) {
      if (file.hasOwnProperty(key)) {
        var value = file[key];
        if (fns) this.route(value.path).all(fns);
        this.handle(method, value, this.handleError(method, {path: key}));
      }
    }
  });
  ...
  app.defineProp('route', function(path) {
  ...
  app.defineProp('param', function(name, fn) {
  ...
  app.defineProp('use', function (fn) {
    var offset = 0, path = '/';
    // default path to '/'
  ...
  utils.methods.forEach(function(method) {
    app.defineProp(method, function(path) {
  ...
  app.defineProp('all', function(path) {
  ...
  app.defineProp('handleError', function handleError(method, template) {
    return function (err) {
      if (err) {
        var res = JSON.stringify(clone(template));
        err.reason = 'Error running ' + method + ' middleware: ' + res;
        console.error(err);
        return err;
      }
    };
