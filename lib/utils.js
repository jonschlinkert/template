...
utils.pickRenderable = function (file, locals, thisArg) {
  debug.utils('[utils] pickRenderable:', arguments);
  var val = null;

  if (!utils.isString(arguments[0])) return val;
  var types = thisArg.templateType.renderable;
  var name = file;
...
utils.pickPartial = function (name, locals, thisArg) {
  debug.utils('[utils] pickPartial:', arguments);
  var val = null;

  if (!utils.isString(arguments[0])) return val;
  var types = thisArg.templateType.partial;
...
utils.pickCached = function (name, locals, thisArg, types) {
