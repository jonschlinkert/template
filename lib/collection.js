...
Collection.prototype = {
  ...
  context: function(ctx, data) {
    // todo: this is quasi-pseudo-code
    if (typeof ctx !== 'object') {
      throw new Error('Collection#context expects the first argument to be the context for a view.');
    }
  ...
  set: function(key, value, options) {
    value.contexts = utils.extend({}, value.contexts, options.contexts);
    value.contexts.create = options;
    value.options = utils.extend({}, options, value.options);
  ...
  render: function () {
    ...
    return view.render.apply(app, args);
  },
  ...
  values: function() {
    var vals = [];
    utils.forOwn(this, function(val, key) {
      vals.push(val);
    });
    return vals;
  }
