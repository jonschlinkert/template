...
utils.baseMethod = function baseMethod(thisArg, method, targetObject) {
...
utils.option = function option(thisArg, prop, value) {
  return utils.baseMethod(thisArg, 'option', thisArg.options)(prop, value);
};

utils.data = function data(thisArg, prop, value) {
  return utils.baseMethod(thisArg, 'data', thisArg.cache.data)(prop, value);
};
...
utils.inherits = function inherits(receiver, provider) {
  ...
  if (provider.prototype === undefined) {
    throw new TypeError('The super constructor to `inherits` must have a prototype.');
  }

  receiver.super_ = provider;
...
utils.slice = function slice(arr, i) {
  ...
  if (!utils.isLoader(args[args.length - 1])) {
