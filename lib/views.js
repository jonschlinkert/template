...
utils.delegate(Views.prototype, {
  ...
  /**
   * Generate list pages based on the number of
   * views in the collection.
   *
   * @param  {View} `list` An instance of a view to use for the list pages.
   * @param  {Object} `locals` Optional locals to use in rendering.
   * @param  {Object} `options` Additional options to use.
   * @param  {Function} `cb` Callback function that returns either an error (`err`) or a collection of list pages (`views`)
   */
  ...
  paginate: function (list, locals, options, cb) {
    ...
    }, list.data, list.locals, locals, options);
