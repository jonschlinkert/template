...
utils.filterLoaders = function filterLoaders(args, cache) {
  ...
  while (len--) {
    ...
    // if the first arg is a string, see if it's a cached loader
    if (i === 0 && type === 'string' && cache[arg]) {
      results.stack.push(arg);

    // if it's a function, it's a loader (done is already popped)
    } else if (typeof arg === 'function') {
      results.stack.push(arg);

    // if it's an array after the first arg, it's a loader
    } else if (i > 0 && type === 'array') {
      results.stack.push(arg);
    } else {
