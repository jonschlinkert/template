...
utils.pickExt = function(template, options) {
  var ext;

  if (isObject(template)) {
    ext = get(template, 'options.engine', true);
    if (!ext) {
      ext = get(template, 'options.ext', true);
    }
  }
...
var argsOfType = function argsOfType(type, args) {
  var first = utils.firstIndexOfType(type, args);
  if (first == null) {
    return null;
  }
