...
defineGetter(Collection.prototype, 'src', function() {
  var name = this.name;
  if (typeof this._.loader === 'undefined') {
    throw new Error('Collection#src: .' + name + '() must be called before `.src()');
  }
  if (this._.loader.type === 'stream') {
    var args = [].slice.call(arguments);
    var stream = this._.loader.apply(this, args);
    stream.on('error', function (err) {
      console.log('Collection#src: .%s, %j, %j', name, err, args);
    });
    return stream;
  }
