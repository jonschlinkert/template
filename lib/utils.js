...
utils.isLayout = function (value) {
  return Boolean(utils.pickFrom(value, 'isLayout', lookupKeys));
};
...
utils.isRenderable = function (value) {
  return Boolean(utils.pickFrom(value, 'isRenderable', lookupKeys));
};
 ...
 * @param  {Function} `callback` Final callback when the stack is done.
 */

utils.runLoaderStack = function (stack, callback) {
  if (typeof callback !== 'function') {
    throw new Error('Expected callback to be a function');
  }

  if (!Array.isArray(stack)) {
    return callback(new Error('Expected stack to be an Array'));
  }
