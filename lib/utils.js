...
utils.pickExt = function(template, options, thisArg) {
  ...
  if (isObject(template)) {
    template = _.merge({options: {}}, template);

    // Engine passed on the template
    if (hasOwn(template, 'engine')) {
      return template.engine;

    // Extension passed on the template
    } else if (hasOwn(template, 'ext')) {
      return template.ext;

    // Engine passed on the template options
    } else if (template.options && hasOwn(template.options, 'engine')) {
      return template.options.engine;

    // Engine passed on the render method options
    } else if (hasOwn(options, 'engine')) {
      return options.engine;

    // If all else fails, return the default.
    } else if (thisArg) {
      return thisArg.option('viewEngine');
    }
  }
