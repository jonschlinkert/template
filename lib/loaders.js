...
mixin('loaderType', function (type) {
  ...
  this._.loaders[type].register('last', loaders[type](this));
});

mixin('getType', function (opts) {
  if (typeof opts === 'string') return opts;
  return opts.type || opts.loaderType || 'sync';
});
...
mixin('load', function (args) {
  var loader = this.getLoader(args.opts);
  var type = this.getType(args.opts);
  if (type === 'async') {
    args.done = args.stack.pop();
  }

  var stack = flatten(args.stack.concat('last'));
  var loaders = loader.buildStack(stack, loader.cache);
  if (type === 'async') {
    loaders.stack = loaders.stack.concat(args.done);
  }

  var load = loader.compose(loaders.stack);
  var res = load.apply(this, args.rest.concat(args.opts));
