...
utils.isPartial = function (template, locals) {
  debug.utils('[utils] isPartial:', arguments);
  var keys = Object.keys(template);
  var val = false;

  for (var i = 0; i < keys.length; i++) {
    var value = keys[i]
    if (value && hasOwn(value, 'options')) {
      if (!hasOwn(value.options, ['isLayout', 'isRenderable'])) {
        val = true;
        break;
      }
    }
